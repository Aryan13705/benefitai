...
import sys
# Files are in the same directory in Vercel function
...
# Start background fetch thread (non-blocking) removed for Vercel compatibility
...
def get_news():
    ...
    if NEWS_CACHE["data"] and (now - NEWS_CACHE["last_update"] < CACHE_DURATION):
        return jsonify(NEWS_CACHE["data"])
        
    # Otherwise, fetch synchronously (will only happen once per hour per warm lambda)
    try:
        print(f"[NEWS] On-demand fetching India Today RSS: {INDIA_TODAY_RSS}")
        import requests
        response = requests.get(INDIA_TODAY_RSS, timeout=8)
        if response.status_code == 200:
            feed = feedparser.parse(response.content)
            if feed.entries:
                news_items = []
    ...
    # Fallback to mock data if fetch fails and cache is empty
    mock_news = [
